...
- `docker-compose/03-secure-3-rack-cluster/` pins its three nodes to `ghcr.io/axonops/development/cassandra:5.0.8-2.0.31-dev-auth-1`, the first build carrying the `CASSANDRA_AUTHENTICATOR` entrypoint support, because the current production image would ignore it and start the cluster unsecured without saying so. Move it to `ghcr.io/axonops/cassandra/cassandra` at the next Cassandra release; `env.example` carries both lines.
