...
### Added
- New standalone Apache Cassandra container image, published from its own pipeline as `ghcr.io/axonops/cassandra/cassandra:{CASSANDRA}-{AGENT}-{BUILD}` (for example `5.0.9-2.0.31-1.0.0`). Built on Red Hat UBI 9 minimal from the official Apache Cassandra tarball with SHA512 verification, carrying the AxonOps agent, cqlai, jemalloc and tini — and no K8ssandra components. Covers Cassandra 5.0.1 through 5.0.9, multi-arch (amd64/arm64), cosign-signed, with floating `{CASSANDRA}-{AGENT}`, `{CASSANDRA}`, `5.0-latest` and `latest` tags. The agent component of a tag is always resolved to a concrete version, never the literal `latest` ([#157](https://github.com/axonops/axonops-containers/issues/157)).
- `cassandra/versions.json` as the single source of truth for the Cassandra build matrix and the tarball checksums, verified by the publish pipeline before any build runs.
- Optional `INSTALL_K8SSANDRA_API` build argument on the new Cassandra Dockerfile, producing a variant that also carries the K8ssandra Management API copied from the pinned k8ssandra base image. Off by default; with it off the k8ssandra base image is never pulled and no Management API components are present. Not published by the pipeline yet.
