...
curl -I https://github.com/axonops/cqlai/releases/download/v0.2.0/cqlai-0.2.0-1.x86_64.rpm
```

**Issue: Build args not passed correctly**
```bash
# Verify build args in image
docker build --build-arg CASSANDRA_VERSION=5.0.6 --build-arg CQLAI_VERSION=0.2.0 -t test .
...
5. **Test:**
   ...
   docker build --build-arg CASSANDRA_VERSION=5.0.7 --build-arg CQLAI_VERSION=0.2.0 -t test .
...
- `AXONDB_TIMESERIES_CQLAI_VERSION` - cqlai version to install (e.g., `0.2.0`)
